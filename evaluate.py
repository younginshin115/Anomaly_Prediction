            ...
            if not model:
                ...
                if cfg.show_curve:
                    if not os.path.exists('/videos'):
                        print(f" [*] Make directories : /videos")
                        os.makedirs('/videos')
                             
                    video_writer = cv2.VideoWriter(f'/videos/{name}_video.avi', fourcc, 30.0, cfg.img_size)
                    curve_writer = cv2.VideoWriter(f'/videos/{name}_curve.avi', fourcc, 30.0, (600, 430))
                ...
                if cfg.show_heatmap:
                    heatmap_writer = cv2.VideoWriter(f'/videos/{name}_heatmap.avi', fourcc, 30.0, cfg.img_size)
